...
--[[	if self.FinishedGrowing then 
	...
	if self.TimeToNextGrow <= 0 and not self.FinishedGrowing then
		self:Grow()
		self.TimeToNextGrow = randomRange(self.GrowInterval) + self.TimeToNextGrow -- Add negative value
...
function SuperTree:PlantSeed(cframe, distUpBranch)
	if not self.IsABranch then
		cframe = cframe * CFrame.Angles(0, math.rad(randomRange(self.TrunkAngleTheta)), math.rad(randomRange(self.TrunkAnglePhi)))
	end
	...
	--[[else
		...
		self.Model = self.BranchParentTree.Model
		self.LeafModel = self.BranchParentTree.LeafModel
		...
		self.CutEvent = self.BranchParentTree.CutEvent
		--self.CutConnection = self.CutEvent.OnServerEvent:Connect(function(...) self:GetCut(...) end)
...
function SuperTree:Age(timePassed)
	...
	--if self.Billboard:FindFirstChild("Text") then
	--	self.Billboard.Text.Text = math.ceil(self.TimeUntilDeath)
	--else
		--warn("No billboard text found")
	--end
	--print("Death: "..self.TimeUntilDeath)
	...
	elseif self.AdultStage == "Leaves fallen" then --Leaves fallen, wait to fall apart
		if self.TimeUntilDeath <= 0  then
			...
			for i, _ in pairs(self.Cuts--[[cutsAboveCut]]) do --Destroy cuts above the cut
				if self.Cuts[i] then
					for _, face in pairs(self.Cuts[i].Faces) do
						if face.Part then
							face.Part:Destroy()
						end
					end
...
function SuperTree:Grow(timePassed)
	...
	self.GrowCalls = self.GrowCalls + 1 
		...
		if section and section.IsGrowing then
			...
			if section.IsExtremety and not stopForBranches and self:ConeCheck(section) then
				...
				elseif section.DistanceToBend <=0 and section.Length > 2 then --Time to bend
					self.Sections[ID].IsExtremety = false
					self:Split(section, "Bend", section.Length, {min=1,max=1}, self.BendAngle, self.BendUnitYComponentConstraints, {min=0,max=360})
...
function SuperTree:GetVolume()
	...
	for ID = 1, existingSections do
		local section = self.Sections[ID]
		if section and section.Part then
			volume = volume + section.Part.Size.X * section.Part.Size.Y * section.Part.Size.Z
		end	
	end
...
function SuperTree:ConeCheck(section, nosectioncframe)
	...
	for rho=0, math.rad(self.SpaceCheckCone.Angle),5 do
		for theta=0, math.pi * 2, math.pi * 2 / numThetaChecks do
			
			local unit=(checkCFrame * CFrame.Angles(0, theta, rho) * CFrame.Angles(math.pi/2,0,0) * CFrame.new(0, 1, 0)).lookVector
			local ray = Ray.new(checkCFrame.p, unit * self.SpaceCheckCone.Distance)
			
			if workspace:FindPartOnRayWithIgnoreList(ray, coneIgnoreList) then
				if section then
...
function SuperTree:Split(parentBranch, type, distUpBranch, numSplits, splitAngle, unitYComponentConstraints, allowableAngleBetweenSplits)
	...
	--if self.MaxGrowLength - (parentBranch.Length + parentBranch.AncestryLength)	< self.NewBranchCutoff then
	local diff1 = self.MaxGrowCalls - self.GrowCalls
	local diff2 = self.NewBranchCutoff
	if self.BranchParentTree then
		diff2 = self.BranchParentTree.MaxGrowCalls - self.BranchParentTree.GrowCalls
	end
	...
	local newCFrames = {}
	for i = 1, self.NumNewSegmentAttempts do
		
		local success = true
		for j = #newCFrames + 1, numSplits do
			...
			for _, cframe in pairs(newCFrames) do --Check that section attempts do not violate bounds for allowable angle between splits
				...
				if d < splitDistanceBounds.min or d > splitDistanceBounds.max then
					...
					break --Throw out new branch, clean out list and start over
				end
			end
...
function SuperTree:NewSection(cframe, originType, distanceUpParentBranch, parentId, bendAngle)
	...
	if originType == "Branch" then
		...
		local skipTrunkYield = branch.skipTrunkYield
		
		newTree = newTree.new(self.Model, self, parentSection)
		newTree.SkipTrunkYield = skipTrunkYield

		newTree:PlantSeed(cframe)
	
		local idValue = Instance.new("IntValue", newTree.Model)
		idValue.Name = "ParentTreeSectionId"
		idValue.Value = parentId
		local angleValue = Instance.new("CFrameValue", newTree.Model)
		angleValue.Name = "BranchAngle"
		angleValue.Value = bendAngle
		local distValue = Instance.new("NumberValue", newTree.Model)
	...
	--print("Distance to split: "..section.DistanceToSplit)
	--print("Distance to bend: "..section.DistanceToBend)
	--print("Distance to branch: "..section.DistanceToBranch)
	...
	if parentSection then
		...
		--self.Billboard.Parent = section.Part
	end
...
function SuperTree:NewLeafBunch(section)
	local numLeaves = randomRangeInteger(self.NumLeafParts)
...
cuttingModule = require(game.ServerScriptService.Trees.TreeUtilities.Cutting)

function SuperTree:GetCut(player, ...)
	cuttingModule:GetCut(self, player, ...)
end

function SuperTree:UpdateAllCuts()
	cuttingModule:UpdateAllCuts(self)
end
